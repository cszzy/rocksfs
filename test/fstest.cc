...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    printf("----------\n");
    // Remove file through rmdir should raise error
    ...
    // int ops = 500000;
    // for(int i = 0; i < ops; i++){
    //     std::string file = mntdir + "/" + std::to_string(i);
    //     // regular file
    //     ret = creat(file.c_str(), S_IRWXU | S_IRWXG | S_IRWXO);
    //     assert(ret != -1);
    //     ret = close(ret);
    //     assert(ret == 0);
    // }
    // time_t end = time(NULL);
    // printf("Test pass, IOPS: %f Kops\n", ops / 1000.0 / (end - start));
}
